...
namespace DO { namespace Shakti {
  ...
  void apply_gradient_kernel(Vector2f *dst)
  {
    ...
    Vector2f nabla_f{
      tex2D(in_float_texture, p.x() + 1, p.y()) - tex2D(in_float_texture, p.x() - 1, p.y()),
  ...
  void apply_gradient_polar_coordinates_kernel(Vector2f *dst)
  {
    ...
    const auto f_x = tex2D(in_float_texture, p.x() + 1, p.y()) - tex2D(in_float_texture, p.x() - 1, p.y());
    const auto f_y = tex2D(in_float_texture, p.x(), p.y() + 1) - tex2D(in_float_texture, p.x(), p.y() - 1);
