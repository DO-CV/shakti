...
namespace DO { namespace Shakti {
  ...
  {
    const auto histogram_sizes = Vector3i(256 / quantization_steps[0],
                                          256 / quantization_steps[1],
    ...
    SHAKTI_SAFE_CUDA_CALL(cudaMemcpyToSymbol(Shakti::histogram_sizes,
                                             &histogram_sizes,
                                             sizeof(Vector3i)));

    const auto histogram_padded_width = histogram.padded_width();
    SHAKTI_SAFE_CUDA_CALL(cudaMemcpyToSymbol(Shakti::histogram_padded_width,
                                             &histogram_padded_width,
                                             sizeof(int)));

    SHAKTI_SAFE_CUDA_CALL(cudaMemcpyToSymbol(Shakti::quantization_steps,
                                             &quantization_steps,
                                             sizeof(Vector3i)));
  ...
  void normalize_color_histogram(MultiArray<float, 3>& histogram, int num_pixels)
  {
    SHAKTI_SAFE_CUDA_CALL(cudaMemcpyToSymbol(
      Shakti::num_pixels, &num_pixels, sizeof(int)));
