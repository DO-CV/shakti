...
TEST_F(TestDifferential, test_gradient)
{
  ...
  Image<Vector2f> gradf{ _src_image.sizes() };
  compute_gradient(
    reinterpret_cast<shakti::Vector2f *>(gradf.data()),
    f.data(),
    f.sizes().data());
...
//  compute_laplacian(laplacian.data(), f.data(), f.sizes().data());
